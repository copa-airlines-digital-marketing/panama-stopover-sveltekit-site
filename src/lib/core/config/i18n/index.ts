...
const getPreferredLocale = ({ cookies, request: { headers } }: RequestEvent) => {
	...
	if (isNil(browserPreferredLocales) || isEmpty(browserPreferredLocales)) return siteDefaultLocale;
	...
	if (!preferredLocale) preferredLocale = siteDefaultLocale;
...
export { getPreferredLocale, isSupportedLocale };
