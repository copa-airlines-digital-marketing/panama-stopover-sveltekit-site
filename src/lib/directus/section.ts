...
const directusSectionItemName = z.union([
...
const sectionContentSchema = z.object({
	...
	display: z
		.union([z.literal('100'), z.literal('75'), z.literal('50'), z.literal('25')])
		.nullable(),
...
const sectionSchema = z.object({
	...
	horizontal_behaviour: z.union([
		z.literal('full'),
		z.literal('contained'),
		z.literal('container-grid')
	]),
...
const getSections = async (filters: DirectusRequestBody) => {
	...
	say('Sections did not comply with the schema', errors?.slice(-1));
	return null;
};

export { sectionSchema, getSections, isSectionSchema };

export type { SectionSchema, SectionContentSchema };
