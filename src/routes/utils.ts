...
import { isStopoverModuleSchema, type StopoverHotelModuleSchema } from "$lib/directus/stopover_hotel_module"
...
const getGroupContentModules = (path: string[]) => (result: Array<string[]>, {item}: ContentGroupContent, key: number) => {
  if(!isStopoverModuleSchema(item))
    return result

  return [...result, [...path, 'content', ''+key, 'item']]
}

const getSectionContentModules = (path: number) => (result: Array<string[]>, {item}: SectionContentSchema, key: number) => {
...
const getSectionModules = (result: Array<string[]>, {section_content}: SectionSchema, key: number) => {
  if(isNil(section_content) || isEmpty(section_content))
    return result
...
const getCollectionOfModule = (collectionName: string) => pathOr(null, [collectionName], collectionMap)

const buildModuleQuery = ({max_items, highlight_only, sort, promo_only, pilar}:StopoverHotelModuleSchema, locale: string): QueryItem<Schema, 'stopover_hotels'> => ({
        fields: [
            'main_image',
            'promo_discount_percent',
            'promo_discount_amount',
...
const getModuleRequest = (sections: SectionContentSchema, locale: string) => (path: string[]) => {
  ...
  return getItems(collectionName, buildModuleQuery(module, locale), null)
}
