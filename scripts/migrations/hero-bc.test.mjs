...
import { createPlan, sourceIds, stageCopies, normalizeCta } from './hero-bc.mjs';
...
function fixture() {
	...
	const references = sourceIds.map((id) => ({
		...
		item: String(id),
		component_name: [118, 142, 144, 161, 183, 184].includes(id) ? 'hero-b' : 'hero-c',
		order: 1,
		display: null,
...
describe('hero-bc content migration', () => {
	...
	it('stages drafts once and verifies reused records on retry', async () => {
		...
		await stageCopies(client, plan);
		expect(client.writes).toHaveLength(12);
		expect(
			client.writes.every(
...
it('only omits the reviewed empty en-ca translation, never populated content', () => {
	const input = fixture();
	const source = input.sources.find((s) => s.id === 126);
	source.translations.push({ languages_code: 'en-ca' });
	expect(
		createPlan(input).entries.find((e) => e.sourceId === 126).omittedTranslations
	).toHaveLength(1);
	source.translations.at(-1).title = 'New copy';
	expect(() => createPlan(input)).toThrow('Unsupported translation contains content');
});

it('anchors relative CTA links to the verified localized parent', () => {
	expect(
		normalizeCta({ text: 'Naturaleza', link: '../naturaleza', open_in: '_self' }, 'es').link
	).toBe('/es/conoce-panama/naturaleza/');
	expect(() =>
		normalizeCta({ text: 'X', link: 'javascript:alert(1)', open_in: '_self' }, 'es')
	).toThrow();
	expect(() => normalizeCta({ text: 'X', link: '//evil.test', open_in: '_self' }, 'es')).toThrow();
});
